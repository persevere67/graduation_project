...
dev_news_path = os.path.join(DATASET_DIR, "MINDsmall_dev", "news.tsv")
...
# 4. 保存核心文件
# 保存全量 Embedding 矩阵
np.save(os.path.join(PROCESSED_DIR, "news_embeddings.npy"), embeddings)

# 保存全量 ID 到索引的映射字典
news_id_dict = {nid: i for i, nid in enumerate(full_news_df["NewsID"])}
with open(os.path.join(PROCESSED_DIR, "news_id_dict.pkl"), "wb") as f:
